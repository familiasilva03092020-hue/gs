...
Mario.Character = function() {
    ...
    this.GroundInertia = 0.8; // tuned for snappier ground control
    this.AirInertia = 0.95; // tuned for smoother air control
...
Mario.Character.prototype.Move = function() {
    ...
    if (Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.S) || (this.JumpTime < 0 && !this.OnGround && !this.Sliding)) {
        if (this.JumpTime < 0) {
            ...
            this.XJumpSpeed = 0;
            this.YJumpSpeed = -2.6; // stronger initial jump
            this.JumpTime = 10; // allow longer variable jump when holding key
            this.Ya = this.JumpTime * this.YJumpSpeed;
            this.OnGround = false;
            this.Sliding = false;
    ...
    this.Ya *= 0.90; // reduced damping for smoother vertical motion
    if (this.OnGround) {
    ...
    if (!this.OnGround) {
        this.Ya += 2.2; // gentler gravity for floatier jumps
    }
